...
// 编辑计划介面
router.get('/:projectId/plans/:planId', function(req, res, next) {
  var projectId = req.params.projectId,
      ...
      if (project) {
        Plans.findById(planId)
          .exec(function(err, plan) {
            ...
            if (plan) {
              project.plan = plan;
              res.render('project/planEdit', {
                title: '计划修改 - ' + project.name,
                project: project
              });
            } else {
              next();
...
// 编辑计划接口
router.put('/:projectId/plans/:planId', function(req, res, next) {
  var projectId = req.params.projectId,
    planId = req.params.planId;
    ...
    .exec(function(err, project) {
      ...
      if (project) {
        _.each(project.plans, function(elem, index, list) {
          var start_at = new Date(elem.start_at)
            .valueOf(),
            complete_at = new Date(elem.complete_at)
            .valueOf();
          ...
          if (!elem.complete) {
            ...
            elem.monitor = _.extendOwn(elem.monitor, {
              start_at: new Date(elem.reality.start_at)
                .valueOf(),
              complete_at: new Date(elem.reality.complete_at)
                .valueOf(),
